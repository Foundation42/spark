...
// SDF lane routing, plus per-glyph attention attribute.
//
...
// `attention` is the LM-driven metadata channel — Phase 6 only the
// SDF branch consumes it (threshold modulation), but other lanes
// will pick it up as effect attributes come online.
...
struct GlyphInstance {
    ...
    float attention;
    uint fx_kind;
    // std430 pads the struct to 64 bytes — Zig side declares an
